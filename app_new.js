(function () {
 ...
 function findLastPlayed(actual) {
  ...
  function actualScoreForPrediction(prediction, actual) {
    if (!prediction || !actual || !actual.byId) return '—';
    const match = actual.byId[prediction.id];
    if (!match || !isMatchFinal(match) || !hasMatchScore(match)) return '—';
    return scoreText(match, { pensOnlyWhenTied: true });
  }
